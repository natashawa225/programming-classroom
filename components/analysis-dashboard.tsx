...
function resolvePalette() {
  ...
  return {
    ...
    q1Line:    get('--primary')              ?? BASE_PALETTE.q1Line,
    q23Line:   get('--accent')               ?? BASE_PALETTE.q23Line,
...
function buildConfidenceData(analysis: AnalysisDashboardAnalysis | null): ConfidencePoint[] {
  ...
  return [1, 2, 3, 4, 5].map(confidence => {
    ...
    const b1 = q1?.confidence_breakdown?.[key]
    const q1Accuracy = b1 && b1.total > 0 ? (toNumber(b1.correct) / b1.total) * 100 : null
    ...
    return { confidence, q1Accuracy, q23Accuracy: total > 0 ? (correct / total) * 100 : null }
  })
}
...
export function AnalysisDashboard({ analysis }: AnalysisDashboardProps) {
  ...
  useEffect(() => {
    if (selectedConfidence === null) {
      const first = confidenceData.find(p => p.q1Accuracy !== null || p.q23Accuracy !== null)
    ...
    <Card className="p-5 space-y-5">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <Swatch color={palette.q1Line}  label="Q1" />
            <Swatch color={palette.q23Line} label="Q2 + Q3" />
          </div>
        </div>
        <div ref={confidenceRef} style={{ height: CHART_H }}>
          {confidenceData.some(p => p.q1Accuracy !== null || p.q23Accuracy !== null) ? (
            <LineChart
              data={confidenceData}
            ...
            >
              ...
              <Line
                type="monotone" dataKey="q1Accuracy" name="Q1"
                stroke={palette.q1Line} strokeWidth={2.5} connectNulls
                dot={{ r: 4, fill: palette.q1Line, strokeWidth: 0 }}
                activeDot={{ r: 6, fill: palette.q1Line, stroke: 'var(--background)', strokeWidth: 2 }}
                isAnimationActive={false}
              />
              <Line
                type="monotone" dataKey="q23Accuracy" name="Q2+Q3"
                stroke={palette.q23Line} strokeWidth={2.5} connectNulls
                strokeDasharray="5 3"
                dot={{ r: 4, fill: palette.q23Line, strokeWidth: 0 }}
                activeDot={{ r: 6, fill: palette.q23Line, stroke: 'var(--background)', strokeWidth: 2 }}
                isAnimationActive={false}
              />
            </LineChart>
          ) : (
          ...
          <div className="rounded-md border border-border/50 bg-secondary/10 px-3 py-2 text-xs">
            ...
            <div className="mt-1 flex gap-4 text-foreground/55">
              <span>Q1: <strong className="text-foreground">
                {selectedPoint.q1Accuracy === null ? 'N/A' : `${selectedPoint.q1Accuracy.toFixed(0)}%`}
              </strong></span>
              <span>Q2+Q3: <strong className="text-foreground">
                {selectedPoint.q23Accuracy === null ? 'N/A' : `${selectedPoint.q23Accuracy.toFixed(0)}%`}
              </strong></span>
            </div>
          </div>
        )}
